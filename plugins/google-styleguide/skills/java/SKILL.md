---
name: google-styleguide-java
description: >-
  Google Java Style Guide. Java naming conventions formatting
  classes methods imports annotations javadoc braces
...
Apply Google's Java style conventions when writing or reviewing
Java code.

## Instructions

1. The full Google Java Style Guide is embedded below
2. If the guide content is missing, tell the user:
   "The Java style guide hasn't been downloaded yet. Run
...
4. When writing code: follow the guide's conventions for naming,
   formatting, imports, javadoc, and other style rules
...
<!-- BEGIN GUIDE CONTENT -->
<!-- Fetched from https://raw.githubusercontent.com/google/styleguide/gh-pages/javaguide.html on 2026-03-04 -->
::::::: {#content}
# Google Java Style Guide
...
1.  The term *class* is used inclusively to mean a normal class, record
    class, enum class, interface or annotation type
    (`@interface`{.prettyprint .lang-java}).
2.  The term *member* (of a class) is used inclusively to mean a nested
    class, field, method, *or constructor*; that is, all top-level
    contents of a class except initializers.
...
For a source file containing classes, the file name consists of the
case-sensitive name of the top-level class (of which there is [exactly
...
1.  All other whitespace characters are escaped in `char` and string
...
`\r`{.prettyprint .lang-java}, `\s`{.prettyprint .lang-java},
`\"`{.prettyprint .lang-java}, `\'`{.prettyprint .lang-java} and
`\\`{.prettyprint .lang-java}), that sequence is used rather than the
corresponding octal (e.g. `\012`{.badcode}) or Unicode
(e.g. `\u000a`{.badcode}) escape.
...
A `package-info.java` file is the same, but without the class
declaration.

A `module-info.java` file does not contain a package declaration and
replaces the class declaration with a module declaration, but otherwise
...
### 3.2 Package declaration {#s3.2-package-declaration}

The package declaration is **not line-wrapped**. The column limit
(Section 4.4, [Column limit: 100](#s4.4-column-limit)) does not apply to
...
Imports are **not line-wrapped**. The column limit (Section 4.4, [Column
limit: 100](#s4.4-column-limit)) does not apply to imports.
...
separates the two groups. There are no other blank lines between
imports.

Within each group the imported names appear in ASCII sort order.
(**Note:** this is not the same as the import *lines* being in ASCII
...
multiple constructors. This rule applies even when modifiers such as
`static`{.prettyprint .lang-java} or `private`{.prettyprint .lang-java}
differ between the methods or constructors.

### 3.5 Module declaration {#s3.5-module-declaration}

#### 3.5.1 Ordering and spacing of module directives {#s3.5.1-ordering-module-directives}
...
class, method, constructor, or switch. Note that, by Section 4.8.3.1 on
[array initializers](#s4.8.3.1-array-initializers), any array
...
- Line break after the closing brace, *only if* that brace terminates a
  ...
  followed by `else`{.prettyprint .lang-java} or a comma.
...
2.  `package`{.prettyprint .lang-java} declarations and imports (see
    Sections 3.2 [Package declarations](#s3.2-package-statement) and 3.3
    [Imports](#s3.3-import-statements)).
3.  Contents of [text blocks](#s4.8.9-text-blocks).
4.  Command lines in a comment that may be copied-and-pasted into a
    shell.
5.  Very long identifiers, on the rare occasions they are called for,
...
**Terminology Note:** When code that might otherwise occupy a single
line is divided into multiple lines, this activity is called
*line-wrapping*.
...
2.  When a line is broken at an *assignment* operator the break
    ...
    - This also applies to the colon in an enhanced `for`{.prettyprint
      .lang-java} (\"foreach\") statement.

3.  A method, constructor, or record-class name stays attached to the
    open parenthesis (`(`{.prettyprint .lang-java}) that follows it.

4.  A comma (`,`{.prettyprint .lang-java}) stays attached to the token
    that precedes it.

5.  A line is never broken adjacent to the arrow in a lambda or a switch
    rule, except that a break may come immediately after the arrow if
    the text following it consists of a single unbraced expression.
    Examples:
...
#### 4.6.1 Vertical whitespace (blank lines) {#s4.6.1-vertical-whitespace}
...
1.  *Between* consecutive members or initializers of a class: fields,
    ...
    - [**Exception:** A blank line between two consecutive fields
      (having no other code between them) is optional. Such blank lines
      are used as needed to create *logical groupings* of
      fields.]{.exception}
    - [**Exception:** Blank lines between enum constants are covered in
      [Section 4.8.1](#s4.8.1-enum-classes).]{.exception}
...
from within literals, comments and Javadoc, a single ASCII space also
appears in the following places **only**.

1.  Separating any keyword, such as `if`{.prettyprint .lang-java},
    `for`{.prettyprint .lang-java} or `catch`{.prettyprint .lang-java},
    from an open parenthesis (`(`{.prettyprint .lang-java}) that follows
    it on that line
2.  Separating any keyword, such as `else`{.prettyprint .lang-java} or
    `catch`{.prettyprint .lang-java}, from a closing curly brace
    (`}`{.prettyprint .lang-java}) that precedes it on that line
3.  Before any open curly brace (`{`{.prettyprint .lang-java}), with two
    exceptions:
    - `@SomeAnnotation({a, b})`{.prettyprint .lang-java} (no space is
      used)
    - `String[][] x = {{"foo"}};`{.prettyprint .lang-java} (no space is
      required between `{{`{.prettyprint .lang-java}, by item 10 below)
4.  On both sides of any binary or ternary operator. This also applies
    ...
    - the arrow in a lambda expression:
      ...
      or switch rule: `case "FOO" -> bar();`{.prettyprint .lang-java}

    but not
    - the two colons (`::`{.prettyprint .lang-java}) of a method
      reference, which is written like `Object::toString`{.prettyprint
      .lang-java}
    - the dot separator (`.`{.prettyprint .lang-java}), which is written
      like `object.toString()`{.prettyprint .lang-java}
...
alignment for its own sake creates future problems. For example,
consider a change that touches only one line. If that change disrupts
the previous alignment, it\'s important \*\*not\*\* to introduce
additional changes on nearby lines simply to realign them. Introducing
formatting changes on otherwise unaffected lines corrupts version
history, slows down reviewers, and exacerbates merge conflicts. These
...
After the comma that follows an enum constant, a line break is optional.
...
#### 4.8.4 Switch statements and expressions {#s4.8.4-switch}

For historical reasons, the Java language has two distinct syntaxes for
`switch`{.prettyprint .lang-java}, which we can call *old-style* and
*new-style*. New-style switches use an arrow (`->`{.prettyprint
.lang-java}) after the switch labels, while old-style switches use a
colon (`:`{.prettyprint .lang-java}).

**Terminology Note:** Inside the braces of a *switch block* are either
one or more *switch rules* (new-style); or one or more *statement
groups* (old-style). A *switch rule* consists of a *switch label*
(`case ...`{.prettyprint .lang-java} or `default`{.prettyprint
.lang-java}) followed by `->`{.prettyprint .lang-java} and an
expression, block, or `throw`{.prettyprint .lang-java}. A statement
group consists of one or more switch labels each followed by a colon,
then one or more statements, or, for the *last* statement group, *zero*
or more statements. (These definitions match the Java Language
Specification,
...
Each switch label starts with this +2 indentation.

In a new-style switch, a switch rule can be written on a single line if
it otherwise follows Google style. (It must not exceed the column limit,
and if it contains a non-empty block then there must be a line break
after `{`{.prettyprint .lang-java}.) The line-wrapping rules of [Section
4.5](#s4.5-line-wrapping) apply, including the +4 indent for
continuation lines. For a switch rule with a non-empty block after the
arrow, the same rules apply as for blocks elsewhere: lines between
`{`{.prettyprint .lang-java} and `}`{.prettyprint .lang-java} are
indented a further +2 relative to the line with the switch label.
...
In an old-style switch, the colon of each switch label is followed by a
line break. The statements within a statement group are indented a
further +2.
...
Within an old-style switch block, each statement group either terminates
abruptly (with a `break`{.prettyprint .lang-java},
`continue`{.prettyprint .lang-java}, `return`{.prettyprint .lang-java}
or thrown exception), or is marked with a comment to indicate that
execution will or *might* continue into the next statement group. Any
comment that communicates the idea of fall-through is sufficient
(typically `// fall through`{.prettyprint .lang-java}). This special
...
##### 4.8.4.3 Exhaustiveness and presence of the `default` label {#s4.8.4.3-switch-default}

The Java language requires switch expressions and many kinds of switch
statements to be *exhaustive*. That effectively means that every
possible value that could be switched on will be matched by one of the
switch labels. A switch is exhaustive if it has a `default`{.prettyprint
.lang-java} label, but also for example if the value being switched on
is an enum and every value of the enum is matched by a switch label.
Google Style requires *every* switch to be exhaustive, even those where
the language itself does not require it. This may require adding a
`default`{.prettyprint .lang-java} label, even if it contains no code.

##### 4.8.4.4 Switch expressions {#s4.8.4.4-switch-expressions}
...
##### 4.8.5.2 Class, package, and module annotations {#s4.8.5.2-class-annotation-style}

Annotations applying to a class, package, or module declaration appear
...
##### 4.8.6.2 TODO comments {#s4.8.6.2-todo-comments}
...
explanatory string introduced with a hyphen `-`.

The purpose is to have a consistent `TODO` format that can be searched
to find out how to get more details.
...
Modifiers on `requires` module directives, when present, appear in the
following order:
...
#### 4.8.9 Text Blocks {#s4.8.9-text-blocks}

The opening `"""` of a text block is always on a new line. That line may
either follow the same indentation rules as other constructs, or it may
have no indentation at all (so it starts at the left margin). The
closing `"""` is on a new line with the same indentation as the opening
`"""`, and may be followed on the same line by further code. Each line
of text in the text block is indented at least as much as the opening
and closing `"""`. (If a line is indented further, then the string
literal defined by the text block will have space at the start of that
line.)

The contents of a text block may exceed the [column
limit](#columnlimit). []{#naming}
...
#### 5.2.1 Package and module names {#s5.2.1-package-names}

Package and module names use only lowercase letters and digits (no
underscores). Consecutive words are simply concatenated together. For
example, `com.example.deepspace`, not `com.example.deepSpace`{.badcode}
or `com.example.deep_space`{.badcode}.
...
- A single capital letter, optionally followed by a single numeral (such
  as `E`{.prettyprint .lang-java}, `T`{.prettyprint .lang-java},
...
4.  Finally, join all the words into a single identifier. Note that the
    casing of the original words is almost entirely disregarded.

In very rare circumstances (for example, multipart version numbers), you
may need to use underscores to separate adjacent numbers, since numbers
do not have upper and lower case variants.
...
Examples:
  ...
  \"Turn on 2SV\"         `turnOn2sv`{.prettyprint           `turnOn2Sv`{.badcode}
                          .lang-java}                        

  \"Guava 33.4.6\"        `guava33_4_6`{.prettyprint         `guava3346`{.badcode}
                          .lang-java}                        
  ----------------------- ---------------------------------- -------------------------------
...
a superclass method, a class method implementing an interface method, an
interface method respecifying a superinterface method, and an explicitly
declared accessor method for a record component.
...
\"impossible\", rethrow it as an `AssertionError`{.prettyprint
.lang-java}.)
...
Do not override `Object.finalize`{.prettyprint .lang-java}. Finalization
support is [*scheduled for removal*](https://openjdk.org/jeps/421).
...
there are no block tags such as `@param`.
...
`/** Returns the customer ID. */`{.prettyprint .lang-java} or
`/** {@return the customer ID} */`{.prettyprint .lang-java}.

[]{#s7.3.3-javadoc-optional}

### 7.3 Where Javadoc is used {#s7.3-javadoc-where-required}

At the *minimum*, Javadoc is present for every *visible* class, member,
or record component, with a few exceptions noted below. A top-level
class is visible if it is `public`{.prettyprint .lang-java}; a member is
visible if it is `public`{.prettyprint .lang-java} or
`protected`{.prettyprint .lang-java} and its containing class is
visible; and a record component is visible if its containing record is
visible.
...
Javadoc is optional for \"simple, obvious\" members and record
components, such as a `getFoo()`{.prettyprint .lang-java} method, *if*
there *really and truly* is nothing else worthwhile to say but \"the
foo\".
...
For example, for a record component named `canonicalName`{.prettyprint
.lang-java}, don\'t omit its documentation (with the rationale that it
would say only `@param canonicalName the canonical name`{.badcode}) if a
...
Other classes, members, and record components have Javadoc *as needed or
desired*.
