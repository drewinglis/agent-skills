---
name: google-styleguide-typescript
description: >-
  Google TypeScript Style Guide. TypeScript TS types interfaces
  generics enums naming conventions formatting modules imports
  exports decorators type assertions type guards.
manual: false
---

# Google TypeScript Style Guide

Apply Google's TypeScript style conventions when writing or
reviewing TypeScript code.

## Instructions

1. The full Google TypeScript Style Guide is embedded below
2. If the guide content is missing, tell the user:
   "The TypeScript style guide hasn't been downloaded yet. Run
...
4. When writing code: follow the guide's conventions for naming,
   formatting, types, interfaces, and other style rules
...
<!-- BEGIN GUIDE CONTENT -->
<!-- Fetched from https://raw.githubusercontent.com/google/styleguide/gh-pages/tsguide.html on 2026-03-04 -->
::::::::::::::::::::::: {#content}
# Google TypeScript Style Guide
...
- In development mode, we typically want quick iteration loops. The
  compiler transpiles to JavaScript without full type information. This
  is much faster, but requires `import type` in certain cases.
...
- "function expression": an expression, typically used in an assignment
  or passed as a parameter, using the `function` keyword
...
- Do not mark interfaces specially (~~`IMyInterface`~~ or
  ...
  why the interface exists in the first place (e.g. `class TodoItem` and
  `interface TodoItemStorage` if the interface expresses the format used
...
- Suffixing `Observable`s with `$` is a common external convention and
  ...
  values. Judgement on whether this is a useful convention is left up to
  individual teams, but *should* be consistent within projects.
...
- Compared to explicitly specifying properties and type relations (e.g.
  using interfaces and extension, see below for an example), type
  operators require the reader to mentally evaluate the type expression.
  This can make programs substantially harder to read, in particular
  combined with type inference and expressions crossing file boundaries.
...
- Mapped & conditional types are most powerful when deriving types from
  complex and/or inferred types. On the flip side, this is also when
  they are most prone to create hard to understand and maintain
  programs.
...
- Always use the simplest type construct that can possibly express your
  code.
- A little bit of repetition or verbosity is often much cheaper than the
  long term cost of complex type expressions.
...
- `String`, `Boolean`, and `Number` have slightly different meaning from
  the corresponding primitive types `string`, `boolean`, and `number`.
  Always use the lowercase version.
- `Object` has similarities to both `{}` and `object`, but is slightly
  looser. Use `{}` for a type that include everything except `null` and
  `undefined`, or lowercase `object` to further exclude the other
...
- When testing APIs that need to deal with unchecked values at runtime,
  add casts to the expected type or to `any` and add an explanatory
  comment. This limits error suppression to a single expression.
...
- Use `/** JSDoc */` comments for documentation, i.e. comments a user of
  the code should read.
- Use `// line comments` for implementation comments, i.e. comments that
  only concern the implementation of the code itself.
...
3.  **Code should be maintainable in the long term.**
    ...
    - We use software to automate changes to code, so code is
      autoformatted so it\'s easy for software to meet whitespace rules.
    ...
    - We ask users to write tests. Without tests we cannot have
      confidence that changes that we make to the language, don\'t break
      users.
